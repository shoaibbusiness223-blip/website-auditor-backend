...
export async function handleVerifyEmail(
  ...
  try {
    const { user_id, code } = req.body as {
      user_id: string;
      code: string;
    };
  ...
  try {
    ...
    if (requires2FA) {
      ...
      sendSuccess(
        ...
        200,
        'Check your email for a login verification code.'
      );
      return;
    }
...
export async function handleToggle2FA(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    if (enable) {
      ...
      sendSuccess(
        ...
        { pending: true },
        200,
        'Enter the OTP sent to your email to enable 2FA.'
      );
...
export async function handleConfirm2FAEnable(
  req: Request,
  res: Response
): Promise<void> {
  try {
...
export async function handleMe(
  ...
  try {
    ...
    const { data } = await db
      .from('users')
      .select(
        'id, email, full_name, plan, plan_expires_at, audit_count_month, email_verified, two_fa_enabled, created_at'
      )
      .eq('id', req.user!.id)
      .single();
